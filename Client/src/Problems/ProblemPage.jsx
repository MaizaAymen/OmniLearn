...
function ProblemPage() {
  ...
  const allProblemsById = useMemo(() => {
    ...
    if (apiProblem?.id) mergedProblems[apiProblem.id] = apiProblem;
  ...
  useEffect(() => {
    ...
    const fetchProblemById = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(`Problem not found: ${response.status}`);
        const data = await response.json();
        if (active) setApiProblem(data);
      } catch {
        if (active) setApiProblem(null);
      } finally {
        if (active) setIsProblemLoading(false);
      }
    };
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      setOutput(result);
      if (result.success) toast.success("Code executed successfully!");
  ...
  const startVerticalResize = useCallback(
    (e) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const newHeight = ((startHeight / 100) * panelHeight + delta) / panelHeight * 100;
          ...
          <div className="flex items-center gap-2">
            ...
            {!isFreeTier && (
              <button
                ...
                disabled={isRunning || isSubmitting || isCorrecting}
              >
                {isCorrecting ? <Loader2Icon className="size-3.5 animate-spin" /> : <WandSparklesIcon className="size-3.5" />}
                Correct
              </button>
            )}
            <button
              ...
              disabled={isRunning || isSubmitting || isCorrecting}
            >
              {isSubmitting ? <Loader2Icon className="size-3.5 animate-spin" /> : <SendIcon className="size-3.5" />}
              Submit
            </button>
          </div>

          {/* Right: Timer + Reset + AI Mentor */}
          <div className="flex items-center gap-2">
            <button
          ...
          >
            ...
            >
              <CodeEditorPanel
                ...
                isRunning={isRunning}
                readOnly={false}
                languageLocked={!!assignmentLanguageLock || isFreeTier}
            ...
            >
              <div className="flex items-center gap-0 bg-base-100 border-b border-base-300 px-2">
                ...
                >
                  ...
                  {output && (
                    <span
                      className={`ml-1.5 inline-block w-1.5 h-1.5 rounded-full ${output.success ? "bg-success" : "bg-error"}`}
                    />
                  )}
                </button>
              </div>
